...
ProxyTcpManager.prototype.disconnect = defunction(['Command'], '*', function ProxySocketManager(command) {
  const socketId = command.socketId
  const socketIdString = getSocketIdString(socketId)
...
ProxyTcpManager.prototype.forward = defunction(['Command'], '*', function ProxySocketManager(command) {
  const socketId = command.socketId
  const socketIdString = getSocketIdString(socketId)
  const socketOut = this.socketOuts[socketIdString]
  if (!socketOut) {
