...
const ProxyCircuit = module.exports = defunction(['Keypair', '[]ProxyDescriptor'], '*', function ProxyCircuit(keypair, proxyDescriptors) {
  ...
  this.emitter.on('response.tcpConnected', (response) => {
    this.emitter.emit(`response.tcpConnected.${response.socketId}`, response)
  })

  this.emitter.on('response.tcpDisconnected', (response) => {
    this.emitter.emit(`response.tcpDisconnected.${response.socketId}`, response)
  })

  this.emitter.on('response.tcpBackward', (response) => {
    this.emitter.emit(`response.tcpBackward.${response.socketId}`, response)
  })
})
...
ProxyCircuit.prototype.forward = defunction(['Uint8Array', 'Uint8Array'], 'undefined', function disconnect(socketId, data) {
...
ProxyCircuit.prototype.ready = defunction([], '=>ProxyCircuit', function ready() {
  ...
  this.socket.on('end', () => {
    socket.removeListener('data', cryptixListener)
